...
from visual_programming_prompt.rephrase_attack import *
...
def main(cfg, logger):
    ...
    rephrasing_prefix = rephrasing_list[cfg.rephrasings]
    ...
    while True:
        ...
        # print(prompt_assets.keys())
        # print(prompt, prompt_assets)

        # import cv2
        # cv2.imwrite('color_img.jpg', np.transpose(obs["rgb"]["top"], (1, 2, 0)))

        whole_task, templates, task_setting = prepare_prompt(
            prompt, prompt_assets, single_model=single_model_flag, task=cfg.task
        )

        if "scene" not in templates.keys():
            templates["scene"] = np.transpose(obs["rgb"]["top"], (1, 2, 0))
        ...
        # print(whole_task)
        whole_task_rephrase = whole_task
        # whole_task_rephrase = rephrase_attack(rephrasing_prefix, whole_task)

        logger.info(f"The rephrasing type {cfg.rephrasings}")
        logger.info(f"The rephrasing intention {whole_task_rephrase}")
        # print(whole_task_rephrase)
        exec_codes = exec_steps(whole_task, whole_task_rephrase)
        # print(exec_codes, 1)
        if task_id >= 30:
            break
        done = False
        ...
        for partition in partitions:
            for modal in prompt_modal:
                for rephrasing in rephrasings:
                    ...
                    logger_file = (
                        save_dir
                        + "/eval_on_seed_{}_hide_arm_{}_{}_{}_{}_{}_modal.log".format(
                            eval_cfg["seed"],
                            eval_cfg["hide_arm"],
                            partition,
                            task,
